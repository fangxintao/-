...
if __name__ == '__main__':
    ...
    # 训练数据
    training_data = [(
        "FANG Xin tao studies in ritsumeikan university".split(),
        "B-PER I-PER I-PER O O B-ORG I-ORG".split() #['B','I','I',...]
    ), (
        "georgia tech is a university in georgia".split(),
        "B-ORG I-ORG O O O O O".split()
    )]
    ...
    tag_to_ix = {"B-PER": 0, "I-PER": 1, "B-ORG": 2, "I-ORG": 3, "O": 4, START_TAG: 5, STOP_TAG: 6}# tag的字典，每个tag对应一个编码
    ...
    # Check predictions before training
    with torch.no_grad():
        precheck_sent = prepare_sequence("ritsumeikan university is a university".split(), word_to_ix)
        #precheck_sent = prepare_sequence(training_data[0][0], word_to_ix)
        #precheck_tags = torch.tensor([tag_to_ix[t] for t in training_data[0][1]], dtype=torch.long)
        #print(precheck_tags)
        #print(precheck_tags)
        print("the prediction of the scentence [BEFORE] training is:")
        print(model(precheck_sent))
    ...
    with torch.no_grad():
        precheck_sent = prepare_sequence("ritsumeikan university is a university".split(), word_to_ix)
        #precheck_sent = prepare_sequence("georgia".split(), word_to_ix)
        # precheck_tags = torch.tensor([tag_to_ix[t] for t in training_data[0][1]], dtype=torch.long)
        print("the prediction of the scentence [AFTER] training is:")
        # print( "\"", " ".join(training_data[0][0]),"\"")
        # print("[AFTER] training is:")
        print(model(precheck_sent))
